...
# Use a named logger (not the root logger) so Streamlit reruns don't keep
# adding duplicate handlers. The "if not logger.handlers" guard means the
# FileHandler and its timestamped log file are only created ONCE per process,
# exactly like Jochen's script which runs once and exits.
logger = logging.getLogger("GEPTool")

if not logger.handlers:
    # first run of this process - set up both handlers
    strLoggingFile = str(
    ...
    # subsequent Streamlit reruns - reuse existing handlers, retrieve log filename
    for _h in logger.handlers:
        if isinstance(_h, logging.FileHandler):
            strLoggingFile = _h.baseFilename
            break
...
def parse_mdl(mdl_source):
    ...
    for sheet in wb:
        ...
        # strip and lower-case both sides so casing or trailing spaces in the
        # actual Excel sheet name don't cause ignored sheets to slip through
        listIgnoredLower = [s.strip().lower() for s in IGNORED_SHEETS]
        if sheet.title.strip().lower() in listIgnoredLower:
