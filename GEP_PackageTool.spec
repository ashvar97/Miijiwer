...
def safe_metadata(*packages):
    """Copy metadata only for packages that are actually installed."""
    result = []
    for pkg in packages:
        try:
            result += copy_metadata(pkg)
        except Exception:
            print(f"[WARN] Skipping metadata for '{pkg}' — not installed, ignoring.")
    return result

meta_datas = safe_metadata(
    "streamlit",
    "pandas",
    "openpyxl",
    "altair",
